...
def get_unique_adjacent(string):
    ...
    for i in range(length):
        if (string[i] in freq): freq[string[i]] += 1
    ...
    if (length % 2 == 0):
        if (sorted_freq[0][1] > length // 2): return None
    else:
        if (sorted_freq[0][1] > (length // 2) + 1): return None
    ...
    while (queue):
        if (len(queue) == 1):
            if (queue[0][1] == 2):
                ...
                if (res[-1] != queue[0][0]): res += queue[0][0]
                else: res = queue[0][0] + res
                break
            else:
                return None
        ...
        if (len(queue) > 1 and queue[1][1] == 0): queue.pop(1)
        if (len(queue) > 0 and queue[0][1] == 0): queue.pop(0)
