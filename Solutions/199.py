...
def get_min_changes_helper(string, modifications, stack, current):
    ...
    if (string[0] == "("):
        ...
        modifications1, string1 = get_min_changes_helper(string[1:], modifications, stack+["("], current+"(")
        modifications2, string2 = get_min_changes_helper(string[1:], modifications+1, stack, current)

        # returning the required element
        return min([(modifications1, string1), (modifications2, string2)], key=lambda tup: tup[0])
